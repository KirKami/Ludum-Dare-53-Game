            ...
            foreach (GameObject b in celestials)
            {
                if(!a.Equals(b))
                {
                    ...
                    if (IsElipticalOrbit)
                    {
                        ...
                        a.GetComponent<Rigidbody>().velocity += a.transform.right * Mathf.Sqrt((G * GMultiplier * m2) * ((2 / r) - (1 / (r * 1.5f))));
                    }
                    else
                    {
                        ...
                        a.GetComponent<Rigidbody>().velocity += a.transform.right * Mathf.Sqrt((G * GMultiplier * m2) / r);
                    }
                }
            }
        }
        ...
        foreach (GameObject a in celestials)
        {
            foreach (GameObject b in celestials)
            {
                if (!a.Equals(b))
                {
                    ...
                    a.GetComponent<Rigidbody>().AddForce((b.transform.position - a.transform.position).normalized * (G * GMultiplier * (m1 * m2) / (r * r)));
                }
            }
        }
    }
